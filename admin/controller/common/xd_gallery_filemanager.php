<?php
class ControllerCommonXdGalleryFileManager extends Controller
{
    public function index()
    {
        $this->load->language('common/xd_gallery_filemanager');
        ...
        foreach ($images as $image) {
            ...
            if (is_dir($image)) {
                ...
                $data['images'][] = array(
                    ...
                    'href'  => $this->url->link('common/xd_gallery_filemanager', 'user_token=' . $this->session->data['user_token'] . '&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'catalog/'))) . $url, true)
                );
        ...
        $data['parent'] = $this->url->link('common/xd_gallery_filemanager', 'user_token=' . $this->session->data['user_token'] . $url, true);
        ...
        $data['refresh'] = $this->url->link('common/xd_gallery_filemanager', 'user_token=' . $this->session->data['user_token'] . $url, true);
        ...
        $pagination->limit = 16;
        $pagination->url = $this->url->link('common/xd_gallery_filemanager', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);

        $data['pagination'] = $pagination->render();

        $this->response->setOutput($this->load->view('common/xd_gallery_filemanager', $data));
    }
    ...
    public function upload()
    {
        ...
        if (!$this->user->hasPermission('modify', 'common/xd_gallery_filemanager')) {
